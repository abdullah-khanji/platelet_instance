...
def extract_color(image_name, color_name, lower_bound, upper_bound, output_dir):
    # Load the image
    image_path = os.path.join('data', 'test_labels', image_name)
...
color_ranges = {
    "Vessels": {
        ...
        "output_dir": "./data/ts_labels/Vessels/"
    },
    "Mito": {
        "lower": np.array([0, 180, 180], dtype=np.uint8),
        "upper": np.array([100, 255, 255], dtype=np.uint8),
        "output_dir": "./data/ts_labels/Mito/"
    },
    "Alpha": {
        "lower": np.array([0, 100, 0], dtype=np.uint8),
        "upper": np.array([150, 255, 150], dtype=np.uint8),
        "output_dir": "./data/ts_labels/Alpha/"
    }
}

labels = os.listdir('./data/test_labels/')
for image_name in labels:
    for class_name, color_info in color_ranges.items():
        extract_color(image_name, class_name, color_info["lower"], color_info["upper"], color_info["output_dir"])
